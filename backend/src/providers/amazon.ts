...
export class AmazonProvider implements ProductProvider {
  ...
  extractAsin(url: string): string | null {
    try {
      ...
      logger.error('Failed to extract ASIN from URL:', url, err);
      return null;
    }
  }

  async fetchProduct(url: string): Promise<ScrapeResult> {
    ...
    logger.info(`Launching Playwright to scrape Amazon ASIN: ${asin} via URL: ${canonicalUrl}`);
    ...
    try {
      ...
      logger.info(`Scrape success! ASIN: ${asin}, Name: ${finalResult.name}, Status: ${finalResult.stockStatus}, Price: ${finalResult.currentPrice}`);
      return finalResult;
    } catch (error: any) {
      logger.error(`Error scraping Amazon URL ${url}:`, error.message || error);
      throw error;
    } finally {
      await browser.close();
    }
